...
Published images are tagged in three ways: `unstable` follows the latest commit on `master`, `latest` is produced for each release build, and every git tag (for example, `0.5.1`) is published as a matching Docker tag.
